...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            // Disable ambient loop for now to reduce audio system contention
    ...
    private func processDrop(providers: [NSItemProvider]) async {
        ...
        for provider in providers {
            do {
                ...
                // We'll generate artwork directly in the processing step
                // No need to save SVG files separately
                let artworkURL: URL? = nil
                
                // Process audio using our enhanced MacAudioEngine with improved error handling
                do {
                    // Set up error recovery
                    var tempFiles: [URL] = []
                    
                    // Make sure we clean up any temp files in case of errors
                    defer {
                    ...
                    // STEP 1: Generate enhanced artwork
                    print("🎨 Generating enhanced artwork...")
                    var artworkPNGURL: URL? = nil
                    
                    // Get a random art style for variety
                    let artStyles = ArtGenerator.ArtStyle.allCases
                    let randomStyle = artStyles.randomElement()!
                    
                    // Generate PNG data directly
                    if let pngData = artGenerator.generatePNGData(for: glitchedName, style: randomStyle) {
                        let pngURL = fileManager.tempDirectory.appendingPathComponent("\(glitchedName)_artwork_temp.png")
                        try pngData.write(to: pngURL)
                        artworkPNGURL = pngURL
                        // Track for cleanup
                        tempFiles.append(pngURL)
                        print("✅ Generated \(randomStyle.rawValue) style artwork")
                    } else {
                        print("⚠️ Warning: Could not generate artwork image")
                    }
                    ...
                    if success {
                        // STEP 3: Apply metadata with artwork
                        print("📝 Adding metadata and artwork...")
                        ...
                        guard FileManager.default.fileExists(atPath: finalOutputURL.path) else {
                            throw NSError(domain: "HexbloopError", code: 2, userInfo: [NSLocalizedDescriptionKey: "Output file was not created"])
                        }
                        
                        // Check file size to ensure it's not empty
                        let fileAttributes = try FileManager.default.attributesOfItem(atPath: finalOutputURL.path)
                        let fileSize = fileAttributes[.size] as? UInt64 ?? 0
                        
                        guard fileSize > 0 else {
                            throw NSError(domain: "HexbloopError", code: 3, userInfo: [NSLocalizedDescriptionKey: "Output file is empty"])
                        }
                        
                        print("✅ Successfully processed: \(finalOutputURL.lastPathComponent) [\(fileSize) bytes]")
...
// Extension to convert NSImage to PNG data with opaque background
extension NSImage {
    func pngData() -> Data? {
        ...
        // Draw the image on top at full opacity
        self.draw(
        ...
        // Get PNG representation with high quality
